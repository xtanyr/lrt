import { PrismaClient, UserRole, MetricDirection, ReportLinkType, AnalysisSection, TriggerCode } from '@prisma/client';
...
async function main() {
  ...
  await prisma.userCoffeeShopAssignment.upsert({
    where: { userId_coffeeShopId: { userId: leader.id, coffeeShopId: coffeeShop1.id } },
    update: {},
    create: { userId: leader.id, coffeeShopId: coffeeShop1.id },
  });
  console.log('Assigned leader to Coffee Shop 1');
